...
async function filterData(searchParams: { query?: string; filters?: string }) {
  ...
  const searchQuery = (searchParams?.query || "")
    ...
    .filter((value: string) => {
      ...
      if (value.match(/^\s*$/g)) return false;
      return true;
    });
  ...
  Promise.resolve(
    raw.map(async (student) => {
      ...
      const idx = data.findIndex((record) => {
        return (
          record.uid.trim().toLowerCase() ===
          student["USER_ID"].trim().toLowerCase()
        );
      });
      if (idx >= 0) {
        ...
        //Remove before render if profile is hidden
        if (
          !data[idx].showProfile ||
          !data[idx].showDorm ||
          !data[idx].showProfile
        ) {
          data.splice(idx, 1);
      ...
      //implicit else if not then just pull the its record
